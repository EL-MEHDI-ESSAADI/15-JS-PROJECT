...
let inputNum = document.querySelector("#amount");
let subBtn = document.querySelector(".btn");
let articlesCon = document.querySelector(".lorem-text");
...
subBtn.onclick = (e) => {
  ...
  if (isNaN(times) || times <= 0 || times > 9) {
    articlesCon.innerHTML = `<p class="result">${text[Math.floor(Math.random() * text.length)]}</p>`;
    return;
  } else {
    let tempText = text.slice(0,times);
    tempText =  tempText.map((item) => {
      return `<p class="result">${item}</p>`
    })
    articlesCon.innerHTML = tempText.join("")
  }
